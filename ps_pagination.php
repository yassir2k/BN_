...
class PS_Pagination {
	...
	function paginate($usr, $rl) {
		...
		$all_rs = sqlsrv_query($this->conn, $this->sql, $params, $options);
		if (! $all_rs) {
			if ($this->debug)
				echo "SQL query failed. Check your query.<br /><br />Error Returned: " . sqlsrv_error();
			return false;
		}
		$this->total_rows = sqlsrv_num_rows($all_rs );
		...
		$this->sql = "SELECT *
FROM (
SELECT SERNUMBER, AV_CODE, BUS_NAME, STATE_CODE, STATE_ADDRESS,
ROW_NUMBER() OVER (ORDER BY SERNUMBER) AS RowNum
FROM tblBNames WHERE user_id = '$usr' ) AS SOD
WHERE SOD.RowNum BETWEEN ((".$this->offset."-1)*".$this->rows_per_page.")+1
AND ".$this->rows_per_page."*(".$this->offset.")";
if($rl == "admin")
{
	$this->sql = "SELECT *
FROM (
SELECT SERNUMBER, AV_CODE, BUS_NAME, STATE_CODE, STATE_ADDRESS,
ROW_NUMBER() OVER (ORDER BY BUS_NAME) AS RowNum
FROM tblBNames WHERE user_id = '$usr' ) AS SOD
WHERE SOD.RowNum BETWEEN ((".$this->offset."-1)*".$this->rows_per_page.")+1
AND ".$this->rows_per_page."*(".$this->offset.")";
}
		$rs = sqlsrv_query($this->conn, $this->sql);
		if (! $rs) {
			if ($this->debug)
				echo "Pagination query failed. Check your query.<br /><br />Error Returned: " . sqlsrv_error();
			return false;
		}
		return $rs;
	}
